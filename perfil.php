<?php
session_start(); // inicia sessao para controlar acesso

require_once __DIR__ . '/config.php'; // carrega configuracao e conexao

if (!isset($_SESSION['nome_usuario'])) { // se nao estiver logado, volta para login
    header("Location: index.php");
    exit();
}

$nome_usuario = $_SESSION['nome_usuario']; // nome do usuario logado
$user_id = intval($_SESSION['user_id']); // ID do usuario logado (numero)
...
$data_nasc = new DateTime($row_data['DATA_NASC']); // data de nascimento do banco
$idade = $data_nasc->diff(new DateTime())->y; // idade em anos
                        ...
                        <div style="margin: 15px 0;">
                            <div style="width: 120px; height: 120px; border-radius: 50%; overflow: hidden; border: 3px solid #ddd; margin-bottom: 10px; margin-left: auto; margin-right: auto;">
                                <img id="preview-foto" src="<?php 
                                    // Busca foto atual do banco e mostra default se nao tiver
...
<!--  fazer consulta aqui da biografia-->
                        <?php 
                        // consulta a biografia do usuario e mostra se existir
                        ...
                        if ($consulta_bio) {
                            ...
                            if ($row_bio && !empty($row_bio['BIO'])) {
                                echo htmlspecialchars($row_bio['BIO']); // exibe bio preenchida
                            } else {
                                echo "$nome_usuario nao adicionou uma biografia ainda."; // sem bio
                            }
                ...
                <div class="card">
                    ...
                    <?php if ($user_posts && $user_posts->num_rows > 0): // se encontrou postagens do usuario
                        while ($post = $user_posts->fetch_assoc()): ?>
                            <div class="post">
                                <div><?php echo nl2br(htmlspecialchars($post['TEXTO'])); // texto do post ?></div>
                                <?php if (!empty($post['IMAGEM'])): // se tiver imagem anexada, exibe ?>
                        ...
                        <?php endwhile;
                            else: // se nao existir nenhuma postagem do usuario ?>
            ...
            <div class="sidebar-right">
                <div class="card">
                    <h3>Amigos</h3>
                    <ul class="menu-list">
<!-- Fazer consulta tambem aqui pra ver os amigos e exibir em lista de alguma forma -->
                        <li><a href="#">carlos</a></li>
                        <li><a href="#">fernando pessoa</a></li>
                        <li><a href="#">joao machado</a></li>
